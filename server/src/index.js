...
telemetry.game.on("fine",         (fine)     => logIt(`${fine.offence.name}, ${currency()}${fine.amount.toLocaleString()} fine`))
...
telemetry.game.on("refuel-payed", ()         => logIt(`Refuel payed`))
telemetry.game.on("refuel-paid",  (data)     => logIt(`Refuel paid, ${currency()}${data.amount.toLocaleString()}`))
...
telemetry.truck.on("cruise-control-increase", (data)               => logIt(`Cruise control increased to ${data.kph}kph / ${data.mph}mph`))
telemetry.truck.on("cruise-control-decrease", (data)               => logIt(`Cruise control decreased to ${data.kph}kph / ${data.mph}mph`))
telemetry.truck.on("warning",                 (warning, enabled)   => logIt(enabled ? `Warning: ${warning}` : `${warning} warning disabled`))
telemetry.truck.on("emergency",               (emergency, enabled) => logIt(enabled ? `Emergency: ${emergency}` : `${emergency} emergency disabled`))
...
telemetry.truck.on("refuel-started",          ()                   => logIt("Truck refuel started"))
telemetry.truck.on("refuel",                  (curr, prev)         => logIt(`Truck refuel from ${prev.amount} to ${curr.amount}`))
telemetry.truck.on("refuel-stopped",          ()                   => logIt("Truck refuel stopped"))
...
telemetry.navigation.on("speed-limit", (data) => logIt(`Speed limit changed to ${data.kph}kph / ${data.mph}mph`))
